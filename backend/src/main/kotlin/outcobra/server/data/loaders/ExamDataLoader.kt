...
class ExamDataLoader @Inject constructor(val examRepository: ExamRepository) : DataLoader {
    ...
    override fun load() {
        ...
        OOP_EXAM = saveAndLog("KR", INF5G_OOP_2016_2, true)
        MATHS_EXAM = saveAndLog("Goniometrie", BMS5A_MATHS_2018_2, true)
        PROJECT_EXAM = saveAndLog("Präsentation", INF5K_PROJECT_2017_2, true)
        GUP_EXAM = saveAndLog("WW2", BMS5A_GUP_2018_1, true)
        PHYSICS_EXAM = saveAndLog("Statik", BMS5C_PHYSICS_2017_1, false)
        GERMAN_EXAM = saveAndLog("Kulturgeschichte", BMS5C_GERMAN_2017_1, false)
        OOP_DESIGN_EXAM = saveAndLog("Design-Patterns", INF5G_OOP_DESIGN_2016_2, false)
        DATABASES_EXAM = saveAndLog("SQL-Basics", INF5K_DATABASES_2016_2, false)
    }

    private fun saveAndLog(examName: String, schoolClassSemesterSubject: SchoolClassSemesterSubject, withMark: Boolean): Exam {
        val mark = if (withMark) MarkValue(getRandomMark(), getRandomWeight()) else null
        val exam = Exam(examName, LocalDate.now().minusWeeks(1), listOf(), schoolClassSemesterSubject, mark, "")
