        ...
        return InstitutionDto(from.id, from.name, from.schoolClasses.map { schoolClass ->
            SchoolClassDto(schoolClass.id, schoolClass.normalizedName,
                    schoolClass.schoolYears.map { year ->
                        SchoolYearDto(year.id, year.name, year.validFrom, year.validTo,
                                year.semesters.map { semester ->
                                    SemesterDto(semester.id, semester.name, semester.validTo, semester.validFrom,
                                            semester.subjects.map { subject -> SubjectDto(subject.id, subject.name) })
                                })
                    })
        })
    }
